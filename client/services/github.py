...
def generate_jwt_token_for_github_app() -> str:
    ...
    :return: The encoded JWT, suitable for authenticating with the GitHub API.
    :rtype: str
    """
    payload = {
        "iat": int(time.time()),
...
def respond_to_current_comment_thread(comments_url: str):
    access_token = get_github_app_access_token()

    request_headers = { **headers_without_authorization, "Authorization": f"Bearer {access_token}" }

    logging.info(f"Making Github Issue Comment Create(POST) request to {comments_url}")
    response = requests.post(comments_url, headers=request_headers, json={"body": "Thank You For Commenting"})
    response.raise_for_status()
...
def check_if_issue_comment_is_made_by_agent(issue: dict):
        try:
            ...
            if performed_by_github_app is None: return False
            return performed_by_github_app.get("id", None) != None
        except Exception:
            return False
